...
let interval = setInterval(() =>{
	showslide();
},3000); //Change every image after 3 sec

slider.addEventListener("mouseover", () =>{
	clearInterval(interval); //onHover stop Changing every image after 3 sec 
});

slider.addEventListener("mouseout", () =>{
	interval = setInterval(() =>{
		showslide();  //on mouseout from slide then again start changing every image after 3 sec
	},3000);
});
